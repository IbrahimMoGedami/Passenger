...
//        GeneralFunctions.setImgTintColor(imgView: pickUpLocImgView, color: UIColor(hex: 0x1c730b))
    ...
    func setLocationIndicator(){
        ...
        if(self.mainScreenUv != nil && self.mainScreenUv.isDriverAssigned == true && self.mainScreenUv.userProfileJson.get("IS_DEST_ANYTIME_CHANGE").uppercased() == "YES" && self.mainScreenUv.eTripType.uppercased() == Utils.cabGeneralType_Ride.uppercased() && self.mainScreenUv.eFlatTrip == false && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("ePoolRide") == "No" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("iStopId").uppercased() == "" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("eFly").uppercased() == "NO" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("eDestinationMode").uppercased() == "NO"){ /* Pool Condition added for destination.*/ /* MSP CHANGES */
...
//                }
                ...
                self.addDestImgView.image = UIImage(named: "ic_plus-1")!
            }
        ...
        if(mainScreenUv != nil && self.mainScreenUv.isDriverAssigned == false){
            ...
            if(self.mainScreenUv.pickUpAddress != ""){
                ...
                self.addPickUpImgView.image = UIImage(named: "ic_plus-1")!
            }
    ...
    @objc func pickUpViewTappedOnAction(){
...
//                    self.destView.zPosition = -1
    ...
    @objc func destViewTappedOnAction(){
        ...
        if(isPickUpMode == false){
            ...
            if(self.mainScreenUv != nil && (self.mainScreenUv.isDriverAssigned == false || (self.mainScreenUv.isDriverAssigned == true && (self.mainScreenUv.destLocation == nil || self.mainScreenUv.destAddress == "") ))){
               self.continueDestSelection()
                ...
                /* Pool Condition added for destination.*/
                if(self.mainScreenUv.isDriverAssigned == true && self.mainScreenUv.userProfileJson.get("IS_DEST_ANYTIME_CHANGE").uppercased() == "YES" && self.mainScreenUv.eTripType.uppercased() == Utils.cabGeneralType_Ride.uppercased() && self.mainScreenUv.eFlatTrip == false && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("ePoolRide") == "No" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("iStopId").uppercased() == "" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("eFly").uppercased() == "NO" && self.mainScreenUv.userProfileJson.getObj("TripDetails").get("eDestinationMode").uppercased() == "NO"){
...
//                    self.destView.zPosition = 1
                    ...
                    self.view.bringSubviewToFront(self.destView)
...
//                    }
                    ...
                    }else if(self.isDriverAssigned == false){ /* Pool Condition added for destination.*/
                        self.addDestImgView.isHidden = false
                    }
    ...
    func goToPickLoc(){
        ...
        if(self.mainScreenUv.gMapView.minZoom <= maxZoomLevel){
        ...
        self.mainScreenUv.gMapView.animate(with: GMSCameraUpdate.fit(bounds, with: UIEdgeInsets.init(top: self.mainScreenUv.isDriverAssigned == false ? 0 : (self.mainScreenUv.addressBarYPosition + self.mainScreenUv.addressBarHeight + (self.mainScreenUv.isDriverAssigned == true && self.mainScreenUv.isTripStarted == false ? self.mainScreenUv.heightOfWaitingMarker : 0)), left: 0, bottom: 0, right: 0)))
...
//        self.mainScreenUv.gMapView.animate(to: camera)
        ...
        if(self.mainScreenUv.gMapView.minZoom <= self.mainScreenUv.gMapView.maxZoom - 5){
            self.mainScreenUv.gMapView.setMinZoom(self.mainScreenUv.gMapView.minZoom, maxZoom: self.mainScreenUv.gMapView.maxZoom - 5)
        }
        ...
        self.mainScreenUv.gMapView.animate(with: GMSCameraUpdate.fit(bounds, with: UIEdgeInsets.init(top: self.mainScreenUv.addressBarYPosition + self.mainScreenUv.addressBarHeight + (self.mainScreenUv.isTripStarted ? self.mainScreenUv.heightOfWaitingMarker : 0), left: 0, bottom: 0, right: 0)))
        CATransaction.commit()
    ...
    func launchPlaceFinder(centerLocation:CLLocation?){
        ...
        launchPlaceFinder.SCREEN_TYPE = self.isPickUpMode == true ? "PICKUP" : "DESTINATION"
        
        // MSP CHANGES
        launchPlaceFinder.pickUpAddress = self.mainScreenUv.pickUpAddress
        launchPlaceFinder.destAddress = self.mainScreenUv.destAddress
        launchPlaceFinder.pickUpLocation = self.mainScreenUv.pickUpLocation
        launchPlaceFinder.destLocation = self.mainScreenUv.destLocation
        launchPlaceFinder.addressContainerView = self
        ...
        launchPlaceFinder.initializeFinder { (address, latitude, longitude) in
            ...
            if(self.mainScreenUv != nil){
                ...
                if(self.mainScreenUv.isDriverAssigned == false && self.mainScreenUv.loadAvailableCab != nil){
                    self.mainScreenUv.loadAvailableCab.checkAvailableCabs()
                }
                /* MSP Changes */
                let userProfileJson = (GeneralFunctions.getValue(key: Utils.USER_PROFILE_DICT_KEY) as! String).getJsonDataDict().getObj(Utils.message_str)
                if (userProfileJson.get("ENABLE_STOPOVER_POINT").uppercased() == "YES" && self.mainScreenUv.currentCabGeneralType.uppercased() == Utils.cabGeneralType_Ride.uppercased() && self.mainScreenUv.isDriverAssigned == false){
                    ...
                    if(GeneralFunctions.isKeyExistInUserDefaults(key: "MSP_DESTINATIONS") == true){
                        ...
                        if(self.mainScreenUv.categoryContainerView != nil){
                            if(usedArray.count > 1){
                                self.mainScreenUv.categoryContainerView.isHidden = true
                            }else{
                                if self.mainScreenUv.listOfLoadedCategories.count <= 1 {
                                    self.mainScreenUv.categoryContainerView.isHidden = true
                                }else{
                                    self.mainScreenUv.categoryContainerView.isHidden = false
                                }
                            }
                        }
                        
                        if(self.mainScreenUv.isPoolVehicleSelected == true && usedArray.count > 1){
                            self.mainScreenUv.selectedCabTypeId = self.mainScreenUv.getFirstCarTypeID()
                        }
                        self.mainScreenUv.setTripLocationForMSP(selectedAddress: address, selectedLocation: CLLocation(latitude: latitude, longitude: longitude),destAddressArray:usedArray)
                ...
                if(self.mainScreenUv.isFromFly == true){
                    self.mainScreenUv.isFromRequestPickupView = false
                }
                ...
                if (self.mainScreenUv.isPoolVehicleSelected == true){ /* POOL CHANGES*/
                    
                    self.mainScreenUv.poolBackImgTapped()
                    //self.mainScreenUv.loadAvailableCab.checkAvailableCabs()
