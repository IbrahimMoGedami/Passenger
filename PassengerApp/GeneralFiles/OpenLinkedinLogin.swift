...
class OpenLinkedinLogin: NSObject, WKNavigationDelegate {
    ...
    func processData(currLinkedinInst: OpenLinkedinLogin){
        ...
        if #available(iOS 11.0, *) {
            webView = WKWebView.init(frame: CGRect(x: 15, y: (Configurations.isIponeXDevice() == true ? self.uv.view.safeAreaInsets.top + 20 : 20) + 25, width: Application.screenSize.width - 30, height: Application.screenSize.height - 20 - 60))
        } else {
            webView = WKWebView.init(frame: CGRect(x: 15, y: 20 + 25, width: Application.screenSize.width - 30, height: Application.screenSize.height - 20 - 60))
        }
        ...
        self.webView.load(URLRequest(url: URL(string: CommonUtils.linkedin_url)!))
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    if(userProfileJsonDict.get("ONLYDELIVERALL").uppercased() == "YES")
                    {
                        ...
                        {
                            _ = OpenMainProfile(uv: self.uv, userProfileJson: response, checkOutUV: (self.uv as! SignInUV).chackOutUV)
                        }else if self.uv.className == "SignUpUV"{
                            _ = OpenMainProfile(uv: self.uv, userProfileJson: response, checkOutUV: (self.uv as! SignUpUV).chackOutUV)
                        }
