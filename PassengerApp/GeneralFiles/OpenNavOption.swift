...
class OpenNavOption: NSObject {
    ...
    func chooseOption(){
        ...
        bgView.center = CGPoint(x: self.uv.view.bounds.midX, y: self.uv.view.bounds.midY)
         ...
         navigationOptionView = NavigationOptionView(frame: CGRect(x: self.uv.view.frame.width / 2, y: self.uv.view.frame.height / 2, width: (Application.screenSize.width > 390 ? 375 : (Application.screenSize.width - 50)), height: 260))
        ...
        self.navigationOptionView.closeImgView.setOnClickListener { (img) in
            self.removeView()
        }
    }
