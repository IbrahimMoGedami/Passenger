...
//    var currentGeoCodeResult = ""
    ...
    func checkAvailableCabs(){
        ...
        if((destLat == "" || destLong == "") && self.mainScreenUv.isFromFly == true){
            return
        }
        ...
        if(mainScreenUv != nil && mainScreenUv.requestPickUpView != nil){
            ...
            mainScreenUv.cabTypeCollectionView.isHidden = true
        }
        ...
        var iToStationId = ""
        var eFly = "No"
        if(self.mainScreenUv != nil && self.mainScreenUv.isFromFly == true){
            eFly = "Yes"
        }
        if(self.mainScreenUv.pickUpLocationDetail != nil){
            iFromStationId = self.mainScreenUv.pickUpLocationDetail.get("iLocationId")
        }
        if(self.mainScreenUv.dropOffLocationDetail != nil){
            iToStationId = self.mainScreenUv.dropOffLocationDetail.get("iLocationId")
        }
        
        var parameters = ["type":"loadAvailableCab","PassengerLat": "\(self.pickUpLocation!.coordinate.latitude)", "PassengerLon": "\(self.pickUpLocation!.coordinate.longitude)","iUserId": GeneralFunctions.getMemberd(), "PickUpAddress": self.pickUpAddress, "eType": mainScreenUv != nil ? mainScreenUv.currentCabGeneralType : "", "SelectedCabType": mainScreenUv != nil ? mainScreenUv.currentCabGeneralType : "", "sortby" : self.mainScreenUv.selctedFilterType, "DestLat":destLat, "DestLong": destLong, "eFly":eFly, "iFromStationId":iFromStationId, "iToStationId":iToStationId]
//        , "currentGeoCodeResult": currentGeoCodeResult.condenseWhitespace()
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(self.mainScreenUv.requestPickUpView != nil){
                ...
                self.mainScreenUv.cabTypeCollectionView.isHidden = false
            }

            if(self.mainScreenUv != nil){
                self.mainScreenUv.isCabSearching = false
            }
            ...
            if(response != ""){
                ...
                if(self.mainScreenUv != nil){
                    ...
                    if(listOfLoadedCategoriesNames.count > 1 && self.mainScreenUv.currentCabGeneralType == Utils.cabGeneralType_Ride && self.mainScreenUv.isFromFly && self.mainScreenUv.updateMylocPosition == true){
                           self.mainScreenUv.myLocBottomMargin.constant = self.mainScreenUv.myLocBottomMargin.constant + 60
                           self.mainScreenUv.myLocBottomMarginFixed = self.mainScreenUv.myLocBottomMargin.constant + 60
                        self.mainScreenUv.updateMylocPosition = false
                    }
                    ...
                    if(isCarTypeChanged == true){
                        ...
                        if(self.mainScreenUv.requestPickUpView != nil){
                            if self.mainScreenUv.listOfLoadedCategories.count > 1 {
                                ...
                                if(self.mainScreenUv.rentalBackImgView.isHidden == true){
                                     self.mainScreenUv.rentalTapView.isHidden = false
                                }
                            }
