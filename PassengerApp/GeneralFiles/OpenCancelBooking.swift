...
class OpenCancelBooking: NSObject {
    ...
    func cancelTrip(eTripType:String, iTripId:String, iCabBookingId:String, handler: @escaping CompletionHandler){
        ...
        let cancelBookingView = CancelBookingView(frame: CGRect(x: superView.frame.width / 2, y: superView.frame.height / 2, width: (Application.screenSize.width > 390 ? 375 : (Application.screenSize.width - 50)), height: 210))
        ...
        GeneralFunctions.setImgTintColor(imgView: cancelBookingView.arrowImgView, color: .lightGray)
        ...
        cancelBookingView.writeReasonView.isHidden = true
    ...
    func openCancelReansList(_ cancelBookingView: CancelBookingView){
        ...
        openListView.selectedItem = cancelBookingView.optionLbl.text!
        ...
        openListView.show(listObjects: reasonList, title: self.generalFunc.getLanguageLabel(origValue: "Select Reason", key: "LBL_SELECT_CANCEL_REASON"), currentInst: openListView, handler: { (selectedItemId) in
          /*  if(selectedItemId == 0){
                if(cancelBookingView.writeReasonView.isHidden == false){
                ...
                cancelBookingView.writeReasonView.isHidden = false
                UIView.animate(withDuration: 0.2, delay: 0, options: .curveEaseInOut, animations: {
                    cancelBookingView.frame.size.height = 280
                })
