...
class GetUserData: NSObject {
    ...
    func setDeliveryTripId(tripId:String){
        self.deliveryTripId = tripId
    }
...
//        }
        ...
        let parameters = ["type":"getDetail","UserType": Utils.appUserType, "AppVersion": Utils.applicationVersion(), "vDeviceType": Utils.deviceType, "iUserId": GeneralFunctions.getMemberd(), "LiveTripId":self.deliveryTripId]
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    {
                        _ = OpenMainProfile(uv: self.uv, userProfileJson: response, window: self.window!,deliveryTripId:self.deliveryTripId)
                    ...
                    if(dataDict.get("isAppUpdate") != "" && dataDict.get("isAppUpdate") == "true"){
                        ...
                        self.generalFunc.setAlertMessage(uv: self.uv, title: "", content: self.generalFunc.getLanguageLabel(origValue: "Please try again.", key: "LBL_TRY_AGAIN_TXT"), positiveBtn: self.generalFunc.getLanguageLabel(origValue: "Retry", key: "LBL_RETRY_TXT"), nagativeBtn: "", completionHandler: { (btnClickedIndex) in
