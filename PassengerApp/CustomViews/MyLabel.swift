...
//                                 (Application.screenSize.height / 2)  - (self.frame.height / 2))
    ...
    func addImage(originalText:String, image:UIImage?, color:UIColor, position:IconPosition, _ customY:CGFloat = -2, _ customX:CGFloat = 0){
        
        let textStr = NSMutableAttributedString(string: originalText)
        
        // create our NSTextAttachment
        let imgAttachment = NSTextAttachment()
        imgAttachment.bounds = CGRect(x: customX, y: customY, width: image?.size.width ?? 0.0, height: image?.size.height ?? 0.0)
