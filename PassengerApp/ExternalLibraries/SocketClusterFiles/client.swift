...
public class ScClient : Listener, WebSocketDelegate {
        ...
        if (text == "#1") {
            ...
            if let messageObject = JSONConverter.deserializeString(message: text) {
                if let (data, rid, cid, eventName, error) = Parser.getMessageDetails(myMessage: messageObject) {
                    ...
                    case .publish:
                        if let channel = SCModel.getChannelObject(data: data as AnyObject) {
    ...
    convenience init(url : String, authToken : String? = nil) {
        self.init(url: "")
        self.authToken = authToken
        counter = AtomicInteger()
        guard let urll = URL(string: url) else { return }
        let request = URLRequest(url: urll)
        self.socket = WebSocket(request: request)   //URLRequest, url: URL(string: url)!
        
        
        socket.delegate = self
    }
    
    init(urlRequest : URLRequest, authToken : String? = nil, protocols : [String]? = nil) {
        
        counter = AtomicInteger()
        self.authToken = authToken
        socket = WebSocket(request: urlRequest)//, protocols : protocols
        super.init()
        socket.delegate = self
    }
...
//    }
    ...
    private func sendHandShake() {
        let handshake = SCModel.getHandshakeObject(authToken: self.authToken, messageId: counter.incrementAndGet())
    ...
    private func ack(cid : Int) -> (AnyObject?, AnyObject?) -> Void {
        return  {
            ...
            let ackObject = SCModel.getReceiveEventObject(data: data, error: error, messageId: cid)
            self.socket.write(string: ackObject.toJSONString()!)
        }
    }
    
    public func emit (eventName : String, data : AnyObject?) {
        let emitObject = SCModel.getEmitEventObject(eventName: eventName, data: data, messageId: counter.incrementAndGet())
    ...
    public func emitAck (eventName : String, data : AnyObject?, ack : @escaping (String, AnyObject?, AnyObject?)-> Void) {
        ...
        let emitObject = SCModel.getEmitEventObject(eventName: eventName, data: data, messageId: id)
        putEmitAck(id: id, eventName: eventName, ack: ack)
        self.socket.write(string : emitObject.toJSONString()!)
    }
    
    public func subscribe(channelName : String, token : String? = nil) {
        let subscribeObject = SCModel.getSubscribeEventObject(channelName: channelName, messageId: counter.incrementAndGet(), token : token)
    ...
    public func subscribeAck(channelName : String, token : String? = nil, ack : @escaping (String, AnyObject?, AnyObject?)-> Void) {
        let id = counter.incrementAndGet()
        let subscribeObject = SCModel.getSubscribeEventObject(channelName: channelName, messageId: id, token : token)
        putEmitAck(id: id, eventName: channelName, ack: ack)
        self.socket.write(string : subscribeObject.toJSONString()!)
    }
    
    public func unsubscribe(channelName : String) {
        let unsubscribeObject = SCModel.getUnsubscribeEventObject(channelName: channelName, messageId: counter.incrementAndGet())
    ...
    public func unsubscribeAck(channelName : String, ack : @escaping (String, AnyObject?, AnyObject?)-> Void) {
        let id = counter.incrementAndGet()
        let unsubscribeObject = SCModel.getUnsubscribeEventObject(channelName: channelName, messageId: id)
        putEmitAck(id: id, eventName: channelName, ack: ack)
        self.socket.write(string : unsubscribeObject.toJSONString()!)
    }
    
    public func publish(channelName : String, data : AnyObject?) {
        let publishObject = SCModel.getPublishEventObject(channelName: channelName, data: data, messageId: counter.incrementAndGet())
    ...
    public func publishAck(channelName : String, data : AnyObject?, ack : @escaping (String, AnyObject?, AnyObject?)-> Void) {
        let id = counter.incrementAndGet()
        let publishObject = SCModel.getPublishEventObject(channelName: channelName, data: data, messageId: id)
        putEmitAck(id: id, eventName: channelName, ack: ack)
        self.socket.write(string : publishObject.toJSONString()!)
    }
